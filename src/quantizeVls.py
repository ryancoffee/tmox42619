...
def main():
    ...
    for fname in fnames:
        ...
        if m:
            vlspitchcorrect = 150 if int(m.group(1))<87 and int(m.group(1))>81 else 0
    ...
    for i,k in enumerate(portkeys):
        X,Y = np.meshgrid(quants[k].binedges()[1:-crop-1],np.arange(nvlsbins+1))
        if donorm:
            for v in range(nvlsbins):
                if vlsnorm[v]>0:
                    ...
                    hist[k][v,:] *= 0
        ax[i//4,i%4].pcolor(X,Y,np.log2(hist[k][:,1:-crop]))#,origin='lower')
    ...
    for i,k in enumerate(portkeys):
        X,Y = np.meshgrid(np.arange(len(quants[k].binedges())-crop-1),np.arange(nvlsbins+1))
        if donorm:
            for v in range(nvlsbins):
                if vlsnorm[v]>0:
                    hist[k][v,:] /= vlsnorm[v]
