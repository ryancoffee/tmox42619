...
def processmultitofs_log(fnames,portnum):
    ...
    for fname in fnames:
        ...
        for i,p in enumerate(peaks):
            ...
            tmp = data['vls']['data'][i,vlsinds[-1]]   - np.max(data['vls']['data'][i,:20])
            tmp *= (tmp>0)
        ...
        for i in range(len(tofnedges)):
            if tofnedges[i] > 0:
                #tmp = tof[ int(tofaddresses[i]):int(tofaddresses[i]+tofnedges[i]) ] - 25725
                tmp = tof[ int(tofaddresses[i]):int(tofaddresses[i]+tofnedges[i]) ] - t0['port_%i'%portnum]
                #tmp = tof[ int(tofaddresses[i]):int(tofaddresses[i]+tofnedges[i]) ] - t0
